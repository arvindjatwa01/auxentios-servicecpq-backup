...
function ToolBar(props) {
  return (
    <GridToolbarContainer className="row">
      <div className="col-md-6 col-lg-8 font-weight-500 my-1">
        PRICE METHODS
      </div>
    </GridToolbarContainer>
  );
...
function CustomAdjustedPrice(params) {
  ...
  return (
    <GridEditInputCell
      {...params}
      disabled={row.pricingMethod !== "FLAT_RATE"}
    />
  );
}
...
function CustomAmountPercentage(params) {
  ...
  return (
    <GridEditInputCell
      {...params}
      disabled={
        row.pricingMethod !== "PER_ON_TOTAL" &&
        row.pricingMethod !== "PER_ON_LABOR"
      }
    />
  );
}
...
export default function PriceMethodTable(props) {
  ...
  const calculationTypes = (priceMethod) => {
    return priceMethod === "PER_ON_TOTAL" || priceMethod === "PER_ON_LABOR"
      ? [
