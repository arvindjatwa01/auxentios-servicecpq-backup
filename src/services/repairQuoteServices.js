...
export const createQuoteVersion = (quoteName, existingVersion, newVersion) => {
  ...
  return new Promise((resolve, reject) => {
    try {
      axios
        .get(((newVersion && newVersion != '') ? CREATE_QUOTE_VERSION(quoteName, existingVersion, newVersion) : CREATE_QUOTE_VERSION_WITHOUT_NEW_VERSION(quoteName, existingVersion)), config)
        .then((res) => {
